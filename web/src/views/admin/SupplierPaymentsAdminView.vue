    ...
    <el-dialog v-model="visible" title="登记付款" width="720px" destroy-on-close>
      <el-form label-width="90px">
        ...
        <el-form-item label="对账单">
          ...
          >
            <el-option
              ...
              :key="s.id"
              :label="`${s.statement_no} · 未付 ${formatMoney(s.remaining_amount)}`"
              :value="s.id"
            />
          </el-select>
        </el-form-item>
        <el-form-item label="逐单核销">
          ...
          <span class="muted" style="margin-left: 8px">供应商未指定分单时请保持关闭</span>
        </el-form-item>
...
function onFormSupplierChange(id: number | null) {
  ...
  void Promise.all([loadOpenAp(), loadStatements()])
}
...
async function loadStatements() {
  ...
  const res: any = await http.get('/account-statements', {
    params: { partner_id: form.supplier_id, direction: 'supplier', page: 1, page_size: 100 },
  })
...
async function openCreate() {
  ...
  selected.value = []
  visible.value = true
  await Promise.all([loadOpenAp(), loadStatements()])
}
...
async function openFromRouteIntent() {
  ...
  filters.supplier_id = partnerId > 0 ? partnerId : null
  await openCreate()
  if (partnerName && !form.supplier_name) form.supplier_name = partnerName
  if (statementId > 0) {
...
async function submit() {
  const allocations = form.allocate_documents
    ? (selected.value.length ? selected.value : openAp.value.filter((r) => r.alloc > 0))
  ...
  const first = allocations.length
    ? openAp.value.find((x) => x.id === allocations[0].payable_id)
    : null
