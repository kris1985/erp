...
> 已有订单承诺、采购保供、排产产能、生产质量预警、交付经营、问数、损耗/报价辅助等能力不在此重复。以下聚焦跨单据状态断链、影响范围与可审批任务，不把 Agent 做成另一个聊天入口。

### 第一批：规则明确、低风险、高复用

- [ ] 技术资料交接守门：生产启动前核对纸样、尺码表、工艺单、确认样、包装要求与责任人；缺项生成催办清单，禁止 Agent 自动开工或改单
- [ ] 漏扫与账实断链稽核：识别流转卡、领退料、报工、返修、入库之间不可能或缺失的状态跳转；输出待核清单，不自动补账
- [ ] 月结前业务闭环遗留清账：识别已完工未入库、已出货未开票、已收款未核销、长期不动在制、未关闭返修等，并按责任部门形成处理队列

### 第二批：跨批次 / 跨版本关联

- [ ] 主数据漂移巡检：识别 BOM、工艺路线、标准工时、报价与近期实际的偏离，圈定受影响订单；只生成工程修订草稿，确认后才生效
- [ ] 批次质量闭环：关联来料批次、IQC 缺陷、工序不良、返修与供应商，定位疑似根因和隔离范围；输出加严检验 / 隔离 / 索赔草稿
- [ ] 交付资料完备性检查：发运前核验齐码、装箱、箱唛、客户标签、检验报告、报关 / 随货资料与发运窗口，生成缺项催办与发运 checklist

### 第三批：需要组织规则与审批

- [ ] 已承诺变更影响单：客户改色码、交期或包装后，计算库存、采购、在制、排产、箱唛与利润影响；生成逐项审批的变更执行包
- [ ] 技能与人员配置建议：基于技能、认证、近期返工 / 待工和工序负荷推荐补位、培训或换线候选；主管确认后才调班
- [ ] 客诉闭环与设计回灌：将客诉、退货、返修关联至批次、工序、供应商和 BOM 版本，识别重复模式；输出 8D / CAPA 草稿，由质量负责人批准

### 共用能力（先做平台，不重造 9 个 Agent）

- [ ] 定义跨单据状态机一致性规则：声明合法生命周期迁移与约束，供断链稽核和月结清账复用
- [ ] 建立影响图谱：从订单 / 版本 / 批次追溯采购、库存、在制、排产、包装、发运与成本影响
- [ ] 建立责任路由与 SLA：按工厂、部门、订单、物料或工序将异常转为待办并记录处置结果
- [ ] 复用草稿与审批契约：Agent 只可生成清单、变更包、隔离建议和 8D / CAPA 草稿；受控服务与审批人完成写入
