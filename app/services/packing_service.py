...
def _pack_assortment(pattern: list[dict[str, Any]], carton_count: int) -> list[list[dict[str, Any]]]:
    """按配码复制箱：每箱色码数量相同。"""
    n = max(1, int(carton_count))
    box = [
        {
            "color_id": r["color_id"],
        ...
        for r in pattern
        if int(r.get("qty") or 0) > 0 and r.get("size_id")
    ]
    if not box:
        raise PackingError("empty_assortment", "配码为空，无法按订单配码装箱")
    return [[{**ln} for ln in box] for _ in range(n)]


def _resolve_sales_order_line(
    db: Session,
    *,
    ...
    if header and getattr(header, "sales_order_line_id", None):
        line_id = header.sales_order_line_id
    elif order and getattr(order, "sales_order_line_id", None):
        line_id = order.sales_order_line_id
    if not line_id:
        return None
    return db.scalar(
...
def list_header_packing_sources(db: Session, tenant_id: int, header_id: int) -> list[dict[str, Any]]:
    """合并生产单的装箱来源，按销售订单明细行去重。"""
    ...
    rows = db.execute(
        ...
        .join(ExecutionAllocation, ExecutionAllocation.sales_order_line_id == SalesOrderLine.id)
...
def _assortment_from_sales_line(line: SalesOrderLine) -> tuple[list[dict[str, Any]], int, int]:
    """从销售行还原配码与箱数。返回 (配码行, 箱数, 每箱双数)。"""
    boxes = max(1, int(getattr(line, "carton_qty", None) or 1))
    pattern: list[dict[str, Any]] = []
    for it in line.items or []:
        abs_qty = int(it.qty or 0)
        if abs_qty <= 0 or not it.size_id:
            continue
        if abs_qty % boxes != 0:
            raise PackingError(
                "assortment_not_divisible",
                f"色码数量 {abs_qty} 不能被箱数 {boxes} 整除，请检查订单配码",
            )
        pattern.append(
            {
...
def _assortment_label(db: Session, lines: list[dict[str, Any]] | list) -> str:
    ...
    for ln in lines or []:
        ...
        sort = 0
        size = db.get(Size, size_id)
        if size and size.sort_order is not None:
            sort = int(size.sort_order)
        parts.append((sort, str(size_value), qty))
...
def _carton_out(db: Session, c: PackingCarton, *, plan: PackingPlan | None = None) -> dict[str, Any]:
    ...
    if source_so:
        customer_name = source_so.customer_name
        sales_order_no = source_so.order_no
    if header:
        if product is None:
    ...
    if shipment:
        ...
        sales_order_no = sales_order_no or shipment.sales_order_no
    ...
    assortment = _assortment_label(db, lines_out)
    return {
        "id": c.id,
        "plan_id": c.plan_id,
        "seq": c.seq,
        ...
        "warehoused_at": (
            c.warehoused_at.isoformat() if getattr(c, "warehoused_at", None) else None
        ),
...
def _plan_out(db: Session, plan: PackingPlan) -> dict[str, Any]:
    ...
    return {
        ...
        "sales_order_line_id": getattr(plan, "sales_order_line_id", None),
        "order_no": order_no,
        ...
        "assortment": cartons[0].get("assortment") if cartons else "",
        "cartons": cartons,
...
def create_packing_plan(
    ...
    *,
    ...
    if mode == PackingMode.assortment.value:
        sline = db.get(SalesOrderLine, sales_order_line_id) if sales_order_line_id else None
        if sline and header:
            belongs = db.scalar(
                select(ExecutionAllocation.id)
                .join(SpecExecutionOrder, SpecExecutionOrder.id == ExecutionAllocation.execution_id)
                .where(
                    SpecExecutionOrder.header_id == header.id,
                    ExecutionAllocation.sales_order_line_id == sline.id,
                    ExecutionAllocation.tenant_id == tenant_id,
                )
                .limit(1)
            )
            if not belongs:
                raise PackingError("sales_line_mismatch", "所选订单明细不属于该生产单")
        if not sline:
            sources = list_header_packing_sources(db, tenant_id, header.id) if header else []
            if len(sources) > 1:
                raise PackingError("sales_line_required", "该生产单包含多条订单明细，请先选择要打印箱唛的订单明细")
            if len(sources) == 1:
                sline = db.get(SalesOrderLine, sources[0]["sales_order_line_id"])
        sline = sline or _resolve_sales_order_line(db, order=order, header=header)
        if not sline:
            raise PackingError(
                "no_sales_assortment",
        ...
        sales_order_line_id = sline.id
        if header:
            code_prefix = f"{header.header_no}-L{sline.id}"
    else:
    ...
    plan = PackingPlan(
        ...
        execution_id=None,
        header_id=header_id,
        sales_order_id=sline.sales_order_id if mode == PackingMode.assortment.value and sline else None,
...
def list_packing_plans(
    ...
    q = (
        select(PackingPlan)
        .where(PackingPlan.tenant_id == tenant_id)
        .options(selectinload(PackingPlan.cartons).selectinload(PackingCarton.lines))
        .order_by(PackingPlan.id.desc())
    )
    if header_id:
        # 生产单详情需要完整追溯箱唛。按框预装的计划同样会挂 header_id，
        # 不能被 basket_id 过滤掉。
        q = q.where(PackingPlan.header_id == header_id)
    elif order_id:
        q = q.where(PackingPlan.order_id == order_id, PackingPlan.basket_id.is_(None))
    else:
        return []
...
def create_basket_prepack(
    ...
    *,
    ...
    if mode == PackingMode.assortment.value:
        raise PackingError("invalid_mode", "筐预装不支持订单配码，请用单码/混码")
    if pairs_per_carton <= 0:
        raise PackingError("invalid_capacity", "每箱双数须大于 0")
