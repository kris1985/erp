...
def api_list_roles(
    ...
    user: Employee = Depends(require_permissions("menu.roles")),
):
...
def api_create_role(
    body: RoleCreate,
    db: Session = Depends(get_db),
    user: Employee = Depends(require_permissions("btn.roles.write")),
):
...
def api_list_permissions(
    db: Session = Depends(get_db),
    user: Employee = Depends(require_permissions("menu.roles", "menu.permissions")),
):
    return ok(rbac_service.permissions_matrix(db, user.tenant_id))


@router.get("/permissions/tree")
def api_permission_tree(user: Employee = Depends(require_permissions("menu.roles", "menu.permissions"))):
