...
def test_direct_ship_virtual_ledger_and_split_sales_shipments(db):
    ...
    basket_id = cut_cards_for_execution(db, tenant_id=tenant.id, execution_id=exe.id,
                                        dry_run=False, bundle_size=50, mode="basket_bundles")["created"][0]["id"]
    ...
    assert {row["sales_order_no"]: row["qty"] for row in result["produced_splits"]} == {
        "SO-DS-A": 30, "SO-DS-B": 20,
    }
    ...
    assert [(x.direction, int(x.qty)) for x in ledgers] == [("in", 50), ("out", 50)]
